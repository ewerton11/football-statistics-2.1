...
colorMode.addEventListener('click', function () {
  ...
  if (i == 0) {
    ...
    firstelementImg.src = '/src/assets/imagens/claro.png'
    ...
    firstelementImg.src = '/src/assets/imagens/noturno.png'
...
// const updateDate =
// `Ultima atualizaçao ${updatedDay}/${updatedMonth} ${updateTime}:${updateMinutes}`
    ...
    function filterTeam(objectTeam) {
      ...
      if (selectTeam === selectTeam) {
        objectTeam.forEach((item) => {
          ...
          img.src = '/src/assets/imagens/bola-time.png'
...
function fetchData() {
  chartInformation.teams.forEach((item) => {
    objectTeam.push(item)
  })

  chartInformation.teams[0].items.forEach((item) => {
